...
// Line forms:
//
//	evil.exe            plain substring, matched against every column
//	/T[0-9]{4}/         a /regex/, matched against every column
...
func (v *View) compileIOCLine(line string, cased bool, now time.Time) (rowPred, error) {
	...
	// A plain string or /regex/ matched against every column.
	...
	return func(row int, rec []string) bool {
		return v.columnMatch(row, rec, ColAll, m)
	}, nil
}
